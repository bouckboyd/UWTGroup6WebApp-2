...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = search ? `${API_URL}/api/shows?search=${encodeURIComponent(search)}` : `${API_URL}/api/shows`;
    ...
    return NextResponse.json({ success: false, message: error.message || 'Failed to fetch TV shows' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Proxy error:', error);
    return NextResponse.json({ success: false, message: error.message || 'Failed to create TV show' }, { status: 500 });
  }
}
