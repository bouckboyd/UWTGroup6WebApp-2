...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: false, message: error.message || 'Failed to fetch movie' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('Proxy error:', error);
    return NextResponse.json({ success: false, message: error.message || 'Failed to delete movie' }, { status: 500 });
  }
}
